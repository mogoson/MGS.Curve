...
namespace MGS.Curve.Editors
{
    ...
    public class MonoBezierCurveEditor : MonoCurveEditor
    {
        ...
        protected virtual void DrawMoveEditor()
        {
            ...
            DrawFreeMoveHandle(from.point, Quaternion.identity, NODE_SIZE, MoveSnap, SphereCap, position =>
            {
                if (Vector3.Distance(position, Target.To.point) <= NODE_SIZE * GetHandleSize(position))
                {
            ...
            var to = Target.To;
            DrawFreeMoveHandle(to.point, Quaternion.identity, NODE_SIZE, MoveSnap, SphereCap, position =>
            {
                if (Vector3.Distance(position, Target.From.point) <= NODE_SIZE * GetHandleSize(position))
                {
        ...
        protected virtual void DrawTangentEditor()
        {
            ...
            DrawFreeMoveHandle(frTangent, Quaternion.identity, NODE_SIZE, MoveSnap, SphereCap, position =>
            {
            ...
            DrawFreeMoveHandle(toTangent, Quaternion.identity, NODE_SIZE, MoveSnap, SphereCap, position =>
            {
